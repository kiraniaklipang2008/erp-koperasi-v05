-- Phase 3: Initial Data Seeding (Adjusted for existing schema)
...
-- Insert default transaction categories (adjusted for existing schema)
INSERT INTO public.kategori_transaksi (nama, tipe, deskripsi) VALUES
('Simpanan Pokok', 'Simpanan', 'Setoran simpanan pokok anggota'),
('Simpanan Wajib', 'Simpanan', 'Setoran simpanan wajib anggota'),
('Simpanan Sukarela', 'Simpanan', 'Setoran simpanan sukarela anggota'),
('Penarikan Simpanan', 'Penarikan', 'Penarikan simpanan anggota'),
('Pinjaman Reguler', 'Pinjaman', 'Pinjaman reguler anggota'),
('Angsuran Pokok', 'Angsuran', 'Pembayaran angsuran pokok pinjaman'),
('Angsuran Bunga', 'Angsuran', 'Pembayaran bunga pinjaman')
ON CONFLICT DO NOTHING;
...
DECLARE
    next_num INTEGER;
    formatted_num TEXT;
    transaction_field TEXT;
BEGIN
    -- Determine the field name based on table structure
    transaction_field := 'id';
    
    -- Get the next sequence number for today
    SELECT COALESCE(MAX(CAST(SUBSTRING(transaction_field FROM LENGTH(prefix) + 10) AS INTEGER)), 0) + 1
    INTO next_num
